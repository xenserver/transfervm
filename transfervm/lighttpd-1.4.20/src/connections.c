...
int connection_reset(server *srv, connection *con) {
	size_t i;
	//log_error_write(srv, __FILE__, __LINE__, "s", "Connection Reset");
...
int (*connection_session_validator)(server *srv, connection *con) = NULL;
int connection_has_valid_session(server *srv, connection *con)
{
	return connection_session_validator == NULL ?
		0 :
		connection_session_validator(srv, con);
}
